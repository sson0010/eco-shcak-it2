<?php
/**
 * ArileWP Customizer Notification System.
 *
 * @package ArileWP
 */
...
/**
 * ArileWP Customizer Notify Class
 */
class ArileWP_Customizer_Notice {
	...
	/**
	 * The single instance of ArileWP_Customizer_Notice
	 *
	 * @var ArileWP_Customizer_Notice $instance The ArileWP_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main ArileWP_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of ArileWP_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof ArileWP_Customizer_Notice ) ) {
			self::$instance = new ArileWP_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $arilewp_recommended_plugins;
		...
		$arilewp_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$arilewp_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'arilewp-customizer-notice', get_template_directory_uri() . '/inc/customizer/customizer-notice/css/customizer-notice.css', array(), ARILEWP_THEME_VERSION );
		...
		wp_enqueue_script( 'arilewp-customizer-notice', get_template_directory_uri() . '/inc/customizer/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ), ARILEWP_THEME_VERSION );
		wp_localize_script(
			'arilewp-customizer-notice', 'customizer_notice_data', array(
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the ArileWP_Customizer_Notice_Section class.
		 */
		require_once get_template_directory() . '/inc/customizer/customizer-notice/class-customizer-notice-section.php';

		$wp_customize->register_section_type( 'ArileWP_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new ArileWP_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'arilewp_show_recommended_plugins' );
			...
			update_option( 'arilewp_show_recommended_plugins', $show_recommended_plugins );
		}
