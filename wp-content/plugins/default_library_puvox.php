...
var PuvoxLibrary = 
{
	...
	Loader(ShowOrHide, style, content_To_show)
	{
		...
		if (ShowOrHide)
		{
			...
			var content_To_show= (content_To_show || "") + '<br/>' +'<svg xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.0" width="90px" height="90px" viewBox="0 0 128 128" xml:space="preserve"><g><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="1"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.67" transform="rotate(45,64,64)"/><circle cx="16" cy="64" r="16" fill="#ffffff" fill-opacity="0.42" transform="rotate(90,64,64)"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.2" transform="rotate(135,64,64)"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.12" transform="rotate(180,64,64)"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.12" transform="rotate(225,64,64)"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.12" transform="rotate(270,64,64)"/><circle cx="16" cy="64" r="16" fill="'+circlecolor+'" fill-opacity="0.12" transform="rotate(315,64,64)"/><animateTransform attributeName="transform" type="rotate" values="0 64 64;315 64 64;270 64 64;225 64 64;180 64 64;135 64 64;90 64 64;45 64 64" calcMode="discrete" dur="720ms" repeatCount="indefinite"></animateTransform></g></svg>';
			var x = document.createElement('div'); x.id = elementID;    x.setAttribute("style","height:110%; width:110%; position:fixed; top:-5%; left:-5%; z-index:9900; overflow:hidden;");
...
class standard_wp_library__PuvoxSoftware extends standard_php_library__PuvoxSoftware  
{
	...
	{
		$disable_metabox_folding = function ()
		{ ?><script>
	...
	{
		$targetFile = WPMU_PLUGIN_DIR .'/'. basename( dirname( $pluginFile ) .'.php');
		if ( !empty($pages) )
		{
			$content = '<?php
			$disable_plugin = function($plugins){
				$excluded=false;
    ...
    public function jqueryRestore() { 
		$jqueryRestore = function(){ echo  '<script>$=jQuery;</script>'; };
		add_action('admin_head',	$jqueryRestore);
		add_action('wp_head',		$jqueryRestore);
	}
