<?php
/**
 * ArileWP Customizer Notification Section Class.
 *
 * @package ArileWP
 */

/**
 * ArileWP_Customizer_Notice_Section class
 */
class ArileWP_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $arilewp_recommended_plugins as $slug => $plugin_opt ) {
			...
			$arilewp_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$arilewp_recommended_plugin['class'] = 'active';
			} else {
				$arilewp_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$arilewp_recommended_plugin['button_class'] = 'install-now button';
					$arilewp_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$arilewp_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$arilewp_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$arilewp_recommended_plugin['button_class'] = 'deactivate-now button';
					$arilewp_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$arilewp_recommended_plugin['id'] = $slug;
			$arilewp_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$arilewp_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$arilewp_recommended_plugin['description'] = $info->short_description;
			}

			$arilewp_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $arilewp_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="arilewp-recommeded-actions-container arilewp-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="arilewp-recommeded-actions">
